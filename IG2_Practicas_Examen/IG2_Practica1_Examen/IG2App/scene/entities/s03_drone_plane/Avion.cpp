...
Avion::Avion(SceneNode* node) : EntityIG(node, ""), state(State::MOVING), detenido(false), manualControl(true)
{
	...
	pSys->setEmitting(true); // poner a false cuando escena DronesAvion
	...
	//mNode->showBoundingBox(true); // depurar colision
}
...
inline bool Avion::keyPressed(const OgreBites::KeyboardEvent& evt)
{
	...
	if (evt.keysym.sym == SDLK_r) // comentar si escena DronesAvion
	{
